...
def _parse_channels_config() -> List[ChannelPolicyConfig]:
    ...
    if not channels:
        # Channels are optional for wallet-based bot
        logger = __import__('logging').getLogger(__name__)
        logger.info("No channels configured - running in wallet-only mode")
        channels = []
...
def load_config() -> Config:
    ...
    mistral_timeout_secs = int(os.getenv("MISTRAL_TIMEOUT_SECS", "299"))

    # Image upload service
    upload_base = (os.getenv("UPLOAD_BASE") or "http://localhost:8080").rstrip("/")
    
    fee_wallet_address = os.getenv("FEE_WALLET_ADDRESS", "bc1qdw7cav7z9l2675fslaupjxu4ugdn2lz5x8q5e7")
    fee_percentage = float(os.getenv("FEE_PERCENTAGE", "5.0"))
    min_deposit_usd = float(os.getenv("MIN_DEPOSIT_USD", "50.0"))
    ...
    return Config(
        ...
        mistral_timeout_secs=mistral_timeout_secs,

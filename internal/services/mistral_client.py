...
class MistralExtractor:
    def __init__(self, cfg: Config) -> None:
        ...
        self.timeout = cfg.mistral_timeout_secs
        self.base_url = "https://api.mistral.ai".rstrip("/")
    ...
    def extract_signal(
        ...
        if not self.api_key:
            logger.debug("Mistral API key not set; skipping extraction")
            return None
        ...
        logger.info("Mistral extraction payload: %s", json.dumps(payload, indent=2))
        try:
            data = self._request(payload)
            content = data["choices"][0]["message"]["content"]
            response = json.loads(content)
            logger.info("Mistral extraction response: %s", json.dumps(response, indent=2))
            return response
        except Exception as e:
            logger.warning("Mistral extraction failed: %s", e)
            return None
